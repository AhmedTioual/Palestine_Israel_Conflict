    ...
    try:
        ...
        for retry_count in range(max_retries):
            ...
            if download_response.status_code == 200 and download_response.content and get_file_size(download_response) > 3: 
                ...
                cleaned_data = process_population_data(io.BytesIO(download_response.content),year)
...
def automateExtractionData_parallel(year_start,year_end):
    ...
    try:
        with ThreadPoolExecutor(max_workers=20) as executor:
            ...
            for success, cleaned_data in results:
                if success:
                    result = pd.concat([result, cleaned_data], ignore_index=True)
                    #print(f'Data for year {year_result} downloaded successfully')
                else:
                    print(f'Failed to download data')

            result.sort_values(by='year', ascending=True,inplace=True)  
            result.to_excel("C:\\Users\\tioua\\OneDrive\\Desktop\\BI PROJECT\\Data\\NumberOfInhabitants\\IsraeliSettlements_Inhabitants_data.xlsx", index=False)
    except Exception as e:
        print(f"Error: {e}")
